        ...
        .do(function (done) {
          cache.getList(define.cache.users, uids, model.users.getById, function (err, uList) {
            ...
            uList.forEach(function (item, i) {
              list[i].user = item && item.username;
            });
            done();
          });
        })
        ...
        .do(function (done) {
          ...
          cache.getList(define.cache.users, uids, model.users.getById, function (err, uList) {
            ...
            uList.forEach(function (user, i) {
              list[i].user = user && user.username;
            });
            done();
          });
        })
...
exports.query_leaders_list = function (size, page, callback) {
  ...
  cache.get(define.cache.leaders_list, page + '_' + size, function (key, callback) {
    model.posts.getLeadersList(page, size, callback);
  }, function (err, list) {
    if (err) return callback(err);
