  ...
  cache.get(define.cache.posts_list, size + '_' + page, function (key, callback) {
    ...
    cache.getList(define.cache.posts, ids, model.posts.getById, function (err, list) {
      ...
      list.forEach(function (item) {
        return users[parseInt(item.user_id)] = true;
      });
 ...
 * @param {Function} callback
 */
exports.query_comments_tree = function (post_id, comment_id, count, callback) {
  post_id = parseInt(post_id);
  ...
  // Get all comments
  cache.get(define.cache.comments_list, post_id + ':1_' + count, function (key, callback) {
    model.comments.getIdListByPostId(post_id, 1, count, callback);
  }, function (err, ids) {
    if (err) return callback(err);
